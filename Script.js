// script.js (usa module para organización)
...
/* --------------------------
  Carrito: objeto con estado y métodos CORREGIDO
---------------------------*/
const carrito = {
  items: [],
  agregarItem(product){
  ...
  removerItem(productId){
    ...
    this.save();
    this.renderizarCarrito(); // CORREGIDO: actualiza la interfaz
  ...
  if (!contador) {
    ...
    // Crear un contenedor separado para el título y contador
    const tituloContainer = document.createElement('div');
    tituloContainer.className = 'carrito-titulo-container';
    tituloContainer.style.position = 'relative';
    tituloContainer.style.display = 'inline-block';
    
    const carritoTitulo = document.querySelector('.carrito h2');
    if (carritoTitulo) {
      // Mover el título dentro del nuevo contenedor
      carritoTitulo.parentNode.insertBefore(tituloContainer, carritoTitulo);
      tituloContainer.appendChild(carritoTitulo);
